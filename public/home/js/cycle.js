				      ...
				      DrawCircle:function(){
					       ...
					       if(!canvasSupport()){
							...
							if(drawOne=="true"){
								...
								canvas.height=this.PARAMS.w/1.5;
								init=180;
								preM=Math.PI;
							...
							cxt.lineWidth=3;
							var speed=1;
							var radius=this.PARAMS.w/2-55;
							var ball={x:90,y:0,speed:2};
							var T1;
							function drawScreen(){
								...
								cxt.arc(x1,y1,r1-6,310,20,false);
								...
								cxt.beginPath();
								cxt.strokeStyle="#fff";
								// cxt.arc(x1,y1,r1-6,80,initM,false);
								cxt.arc(x1,y1,r1-6,310,initM,false);
								cxt.stroke();  //先执行stroke  就不会出现横线
								cxt.closePath();    
								//画虚线 
								...
								for(var i=initA;i<=init;i+=2){
									...
									cxt.fillStyle="#fff";
									cxt.beginPath();
									cxt.arc(a1,a2,1,Math.PI*0.5,Math.PI*1.5,false);
									//console.log(balls[i].x)
									cxt.closePath();
									cxt.fill();
								}
								...
								cxt.fillStyle="#fff";
								//var messT=parseInt(tip*initM/angle);
								var messT=60;
								// if(drawOne!="ture"){
								...
								var mess=parseInt(messT);

								cxt.fillText(mess,canvas.width/2,canvas.height/2+60);
